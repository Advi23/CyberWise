...
struct PhishingAttackView: View {
    @State private var currentScenario = PhishingScenarios.randomElement()!
        ...
        NavigationView {
            VStack {
                ScrollView{
                    ...
                    NavigationLink(
                        destination: PhishingResultView(isCorrect: isUserCorrect, explanation: explanation, currentScenario: $currentScenario, usedScenarios: $usedScenarios),
    ...
    func handleAnswer(isPhishing: Bool) {
        isUserCorrect = (isPhishing == currentScenario.isPhishing) // Compare with correct answer
        explanation = currentScenario.explanation // Provide the explanation for the scenario
        showResultScreen = true // Show the result screen
    }
}
...
//Sample phishing and non phishing scenarios
...
struct PhishingResultView: View {
    ...
    var body: some View {
        VStack {
            ...
            Spacer()
            //.navigationTitle("Phishing Feedback")
        }
    ...
    // Function to get the next scenario, ensuring it doesn't repeat until all are shown
    func getNextScenario() {
        // Check if all scenarios have been used, then reset
        if usedScenarios.count == PhishingScenarios.count {
            usedScenarios.removeAll()
        }
        
        // Pick a random scenario that hasn't been used
        ...
        NavigationView {
            VStack {
                ScrollView {
                    ...
                    // Navigation to the result screen
                    NavigationLink(
                        destination: PhishingResultView(isCorrect: isUserCorrect, explanation: explanation, getNextScenario: getNextScenario),
                        isActive: $showResultScreen
                    ) {
                        EmptyView()
                    }
